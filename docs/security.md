...
Fresh PostgreSQL initialization separates the schema-owner credential from the API credential. `tyfino_app`-style roles are non-superuser, cannot create roles or databases, cannot alter tables or triggers, receive only the table/column operations used by the licensing service, and have `SELECT` plus business-field `INSERT` access to `audit_logs` without `UPDATE` or `DELETE`. The same idempotent role script removes legacy role memberships and direct schema privileges before reapplying the reviewed matrix; it refuses to continue if the runtime role owns database objects. The API independently refuses startup for a privileged, owning, schema-creating, or role-member database identity. CI starts from an overprivileged existing-role simulation, proves the migration preserves data and removes excess access, runs it twice, rejects the schema owner at API startup, executes the full integration suite through the restricted role, and separately proves that audit mutation and trigger disabling are denied. The schema-owner credential must not be supplied to the API container.
