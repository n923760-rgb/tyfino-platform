...
The Home feature banner selects an already cached resumable Movie first, then a resumable Series episode, then a dated newly added Movie or Series. Its action resumes eligible playback or opens the catalog item's details; artwork is decorative and the title and action remain available if the image fails. This selection makes no new provider request or whole-catalog scan.
